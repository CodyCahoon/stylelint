...
testRule(rule, {
	ruleName,
	config: ['numeric', { ignoreValues: ['bolder', 'lighter'] }],
...
testRule(rule, {
	ruleName,
	syntax: 'scss',
	config: ['numeric', { ignoreValues: [/^map-deep-get/] }],

	accept: [
		{
			code: 'font-weight: map-deep-get($theme, typography, weight, semibold)',
			description: 'ignores sass function provided as a regex pattern',
		},
	],

	reject: [
		{
			code: 'font-weight: some-other-fn($theme, typography, weight, semibold)',
			description: 'non-matching ignored value',
			message: messages.expected('numeric'),
			line: 1,
			column: 14,
		},
